  ...
  return (
    <div className="bg-[#FCFCFC] dark:bg-zinc-950 min-h-screen text-zinc-900 dark:text-zinc-50 font-sans selection:bg-zinc-200 dark:selection:bg-zinc-800 transition-colors duration-300 relative">
      ...
      <section className="pt-28 pb-12 px-6 md:px-12 max-w-[1600px] mx-auto">
        <div className="relative w-full h-[75vh] md:h-[85vh] rounded-3xl overflow-hidden group shadow-sm">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4">
            ...
            <Link to="/shop" className="bg-white text-zinc-900 px-8 py-4 rounded-full text-sm font-semibold hover:bg-zinc-900 hover:text-white transition-all duration-300 hover:shadow-xl hover:-translate-y-1 dark:bg-zinc-900 dark:text-white dark:border dark:border-zinc-700 dark:hover:bg-white dark:hover:text-zinc-900">
              Explorar Colección
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 max-w-[1600px] mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-16 tracking-tight dark:text-white">Comprar por Categoría</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
          {['Hoodies', 'Camisetas', 'Pantalones'].map((cat, i) => (
            <Link to={`/shop/${cat.toLowerCase()}`} key={i} className="group cursor-pointer flex flex-col items-center">
              ...
              <h3 className="text-sm font-semibold uppercase tracking-wider dark:text-zinc-200">{cat}</h3>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-6 md:px-12 max-w-[1600px] mx-auto border-t border-zinc-100 dark:border-zinc-800 transition-colors duration-300">
        <div className="flex justify-between items-end mb-12">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight dark:text-white">Selección Inicial</h2>
          <Link to="/shop" className="text-sm font-medium text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white border-b border-transparent hover:border-zinc-900 dark:hover:border-white transition-all pb-1">
            Ver catálogo
          </Link>
        </div>
          ...
          {[1, 2, 3, 4].map((item) => {
            ...
            return (
              <div key={item} className="group cursor-pointer">
                ...
                <h3 className="text-sm font-medium text-zinc-800 dark:text-zinc-200 mb-1 transition-colors duration-300">Oversize Basic Tee</h3>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 font-semibold transition-colors duration-300">$ 45.00</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="py-20 px-6 md:px-12 max-w-[1600px] mx-auto">
        <div className="bg-zinc-100 dark:bg-zinc-900 rounded-[2rem] overflow-hidden flex flex-col md:flex-row items-center transition-colors duration-300 dark:border dark:border-zinc-800">
          <div className="w-full md:w-1/2 p-12 md:p-20 flex flex-col justify-center">
            ...
            <div>
              <Link to="/lookbook" className="border-2 border-zinc-900 dark:border-white text-zinc-900 dark:text-white px-8 py-3 rounded-full text-sm font-semibold hover:bg-zinc-900 hover:text-white dark:hover:bg-white dark:hover:text-zinc-900 transition-all duration-300 inline-block">
                Ver Lookbook
              </Link>
            </div>
          </div>
      ...
      <section className="py-24 px-6 md:px-12 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800 transition-colors duration-300">
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 text-center divide-y md:divide-y-0 md:divide-x divide-zinc-100 dark:divide-zinc-800">
        ...
        <div className="max-w-[1600px] mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="bg-white dark:bg-zinc-900 p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow text-left dark:border dark:border-zinc-800 transition-colors duration-300">
      ...
      <footer className="bg-white dark:bg-zinc-950 pt-20 pb-10 px-6 md:px-12 border-t border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
        ...
        <div className="max-w-[1600px] mx-auto border-t border-zinc-100 dark:border-zinc-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300">
