...
const AdminDashboard = () => {
  // Estado para controlar en qué pestaña estamos
  const [activeTab, setActiveTab] = useState('productos'); // 'productos', 'categorias', 'usuarios'
  ...
  // Control de las ventanas emergentes (Modales)
  const [isProductModalOpen, setIsProductModalOpen] = useState(false);
  const [isCategoryModalOpen, setIsCategoryModalOpen] = useState(false);
  
  // Formularios
  const [productForm, setProductForm] = useState({ name: '', description: '', price: 0, stock_quantity: 0, category_id: '', image_url: '', is_active: true });
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProductForm({ name: '', description: '', price: 0, stock_quantity: 0, category_id: '', image_url: '', is_active: true });
        cargarProductos();
      }
    } catch (error) {
      console.error(error);
  ...
  // Filtrado de datos según la pestaña activa y la barra de búsqueda
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 pt-0">
          ...
          <div className="flex justify-end gap-3 mb-6">
            ...
            {activeTab === 'productos' && (
              <button onClick={() => setIsProductModalOpen(true)} className="bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 px-6 py-2.5 rounded-full text-sm font-bold shadow-sm hover:scale-105 transition-transform flex items-center gap-2">
                    ...
                    <>
                      <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                        <tr>
                          ...
                          <th className="w-1/2 py-4 px-6 text-xs font-bold uppercase tracking-wider text-zinc-500">Nombre del Producto</th>
                          <th className="w-1/2 py-4 px-6 text-xs font-bold uppercase tracking-wider text-zinc-500">Categoría</th>
                      ...
                      <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/80">
                        {filteredProductos.length === 0 ? (
                          <tr><td colSpan="4" className="py-16 text-center text-zinc-500 text-sm">No hay productos registrados.</td></tr>
                        ) : (
                          filteredProductos.map((p, index) => (
                            <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                              ...
                              <td className="py-4 px-6 font-semibold text-zinc-900 dark:text-white truncate">{p.name}</td>
            ...
            {!isLoading && (
              <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50">
                ...
                <div className="flex gap-2">
                  ...
                  <span className="w-8 h-8 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 rounded-lg text-sm hover:bg-white dark:hover:bg-zinc-800 cursor-pointer transition-colors">2</span>
                  <span className="w-8 h-8 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 rounded-lg text-sm hover:bg-white dark:hover:bg-zinc-800 cursor-pointer transition-colors">3</span>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-lg z-10 overflow-hidden shadow-2xl">
            <div className="px-8 py-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
              ...
              <button onClick={() => setIsProductModalOpen(false)} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors">
            ...
            <form onSubmit={handleProductSubmit} className="p-8 space-y-5">
              ...
              <div>
                ...
                {categorias.length === 0 && <p className="text-xs text-red-500 mt-2">Crea una categoría primero.</p>}
              </div>

              <button type="submit" className="w-full bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 p-4 rounded-xl font-bold mt-2 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors shadow-sm text-sm">Guardar Producto</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-lg z-10 overflow-hidden shadow-2xl">
            <div className="px-8 py-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
              ...
              <button onClick={() => setIsCategoryModalOpen(false)} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors">
                <LogOut size={24} className="rotate-45" />
              </button>
            </div>
            <form onSubmit={handleCategorySubmit} className="p-8 space-y-5">
              <div>
                ...
                <input required placeholder="Ej. Camisetas" value={categoryForm.name} onChange={e => setCategoryForm({...categoryForm, name: e.target.value})} className="w-full p-3 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-zinc-900 dark:focus:border-zinc-400 dark:text-white transition-all text-sm" />
              </div>
              <div>
