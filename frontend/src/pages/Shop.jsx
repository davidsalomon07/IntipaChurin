  ...
  return (
    <div className="bg-[#FCFCFC] dark:bg-zinc-950 min-h-screen text-zinc-900 dark:text-zinc-50 font-sans selection:bg-zinc-200 dark:selection:bg-zinc-800 flex flex-col transition-colors duration-300">
      ...
      <main className="max-w-[1600px] mx-auto px-6 md:px-12 pt-36 pb-24 flex-grow w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-zinc-100 dark:border-zinc-800 pb-6 gap-4 relative">
          <div>
          ...
          <div className="relative w-full md:w-auto">
            <button 
              ...
              className="flex items-center justify-between gap-3 bg-transparent border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 text-sm rounded-full px-5 py-2.5 hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors focus:outline-none w-full md:w-auto md:min-w-[220px]"
            >
            ...
            <div className={`absolute top-full right-0 mt-2 w-full md:min-w-[220px] bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 shadow-xl rounded-2xl py-2 z-20 origin-top-right transition-all duration-200 ${isSortOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-8 md:gap-8">
            {productosFiltrados.map((item) => (
              ...
              >
                <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden bg-zinc-100 dark:bg-zinc-800 mb-3 md:mb-5 relative transition-colors duration-300">
                  <img 
                  ...
                  <div className="absolute bottom-2 right-2 md:bottom-3 md:right-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 md:group-hover:translate-y-0 md:translate-y-2">
                    <button
                      ...
                      className="w-8 h-8 md:w-11 md:h-11 rounded-full flex items-center justify-center shadow-2xl border transform transition-all duration-150 active:scale-90 active:shadow-md bg-zinc-950 text-white hover:bg-zinc-800 border-transparent dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200"
                    >
                ...
                <h3 className="text-[13px] md:text-sm font-medium text-zinc-800 dark:text-zinc-200 mb-0.5 md:mb-1 truncate leading-tight">{item.name}</h3>
                <p className="text-[13px] md:text-sm text-zinc-500 dark:text-zinc-400 font-semibold">$ {parseFloat(item.price).toFixed(2)}</p>
              </div>
            ))}
          </div>
        ) : (
